####! Temp file until we make a separate route53/dns module
####! Temp file until we make a separate route53/dns module
####! Temp file until we make a separate route53/dns module
####! Temp file until we make a separate route53/dns module
####! Temp file until we make a separate route53/dns module
...
resource "aws_route53_record" "default_a_admin" {
    count           = var.active_env_provider == "digital_ocean" && var.dns_provider == "aws_route53" ? length(compact(var.admin_arecord_aliases)) : 0
    zone_id         = data.aws_route53_zone.default.zone_id
...
resource "aws_route53_record" "default_a_db" {
    count           = var.active_env_provider == "digital_ocean" && var.dns_provider == "aws_route53" ? length(compact(var.db_arecord_aliases)) : 0
    zone_id         = data.aws_route53_zone.default.zone_id
...
resource "aws_route53_record" "default_a_leader" {
    count           = var.active_env_provider == "digital_ocean" && var.dns_provider == "aws_route53" ? length(compact(var.leader_arecord_aliases)) : 0
    zone_id         = data.aws_route53_zone.default.zone_id
...
resource "aws_route53_record" "default_a_leader_root" {
    count           = var.active_env_provider == "digital_ocean" && var.dns_provider == "aws_route53" ? 1 : 0
    zone_id         = data.aws_route53_zone.default.zone_id
    name            = var.root_domain_name
    allow_overwrite = true
    ttl             = "300"
