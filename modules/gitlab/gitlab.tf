...
locals {
    ...
    charts = {
        ...
        # Do external pg, redis, gitaly after _everything_ else done, maybe dont bother
        gitlab = {
    ...
    #    password = var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled ? random_password.gitlab_db["user"].result : ""
    #    postgresPassword = var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled ? random_password.gitlab_db["postgres"].result : ""
    #    ## Create from postgres chart
    #    secret = {
    #        name = "postgresql"
    ...
    #}
    #
    #gitlab_global_psql = chomp(local.gitlab_global_psql_heredoc)
    #gitlab_global_psql_heredoc = <<-EOF
    #psql:
    #  host: ${local.gitlab_postgresql.hostname}
    #  database: ${local.gitlab_postgresql.database}
    #  username: postgres
    #  password:
    #    secret: ${local.gitlab_postgresql.secret.name}
    #    key: ${local.gitlab_postgresql.secret.key}
    #EOF
    #  #ssl:
    #  #  secret: ${local.gitlab_postgresql.tls.name}
    #  #  clientCertificate: ${local.gitlab_postgresql.tls.cert2}
    #  #  clientKey: ${local.gitlab_postgresql.tls.key}
    #  #  serverCA: ${local.gitlab_postgresql.tls.ca}

    tf_helm_values = {
        ## TODO: external postgres
        #gitlab_postgresql = <<-EOF
        #image:
        ...
        #auth:
        #  username: ${local.gitlab_postgresql.username}
        #  database: ${local.gitlab_postgresql.database}
        #  password: ${local.gitlab_postgresql.password}
        #  postgresPassword: ${local.gitlab_postgresql.postgresPassword}
        #EOF
        ##tls:
        ##  enabled: ${var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled}
        ##  certificatesSecret: ${local.gitlab_postgresql.tls.name}
        ##  certFilename: ${local.gitlab_postgresql.tls.cert}
        ##  certCAFilename: ${local.gitlab_postgresql.tls.ca}
        ##  certKeyFilename: ${local.gitlab_postgresql.tls.key}
...
#        if var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled
#    }
#    length           = 20
#    special          = false
#}
#resource "tls_private_key" "postgres_ca" {
#    count = var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled ? 1 : 0
#    algorithm = "RSA"
#}
#resource "tls_self_signed_cert" "postgres_ca" {
#    count = var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled ? 1 : 0
#    private_key_pem = tls_private_key.postgres_ca[0].private_key_pem
#    is_ca_certificate = true
#    subject {
#      common_name = local.gitlab_postgresql.hostname
#    }
...
#}
#locals {
#    postgresql_clients = ["gitlab", "gitlab_user"]
#}
#resource "tls_private_key" "postgresql_client" {
#    count = var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled ? 1 : 0
#    algorithm = "RSA"
#}
#resource "tls_cert_request" "postgresql_client" {
#    for_each = {
#        for client in local.postgresql_clients:
#        client => client
#        if var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled
#    }
#    private_key_pem = tls_private_key.postgresql_client[0].private_key_pem
#    subject {
#      common_name = each.key
#    }
#}
#resource "tls_locally_signed_cert" "postgresql_client" {
#    for_each = {
#        for client in local.postgresql_clients:
#        client => client
#        if var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled
#    }
#    cert_request_pem = tls_cert_request.postgresql_client[each.key].cert_request_pem
#    ca_private_key_pem = tls_private_key.postgres_ca[0].private_key_pem
#    ca_cert_pem = tls_self_signed_cert.postgres_ca[0].cert_pem
#    # Certificate expires after ~40000 days.
#    validity_period_hours = 999999
#    allowed_uses = [
#        #"key_encipherment",
...
#    count = var.gitlab_enabled && local.charts["gitlab_postgresql"].enabled ? 1 : 0
#    depends_on = [
#        tls_private_key.postgres_ca,
#        tls_self_signed_cert.postgres_ca,
#
#        tls_private_key.postgresql_client,
#        tls_cert_request.postgresql_client,
#        tls_locally_signed_cert.postgresql_client,
#        kubernetes_namespace.gitlab
#    ]
#    metadata {
#        name = local.gitlab_postgresql.tls.name
#        namespace = "gitlab"
#    }
#    type = "kubernetes.io/tls"
#    data = {
#        "${local.gitlab_postgresql.tls.ca}" = tls_self_signed_cert.postgres_ca[0].cert_pem
#        "${local.gitlab_postgresql.tls.key}" = tls_private_key.postgresql_client[0].private_key_pem
#        "${local.gitlab_postgresql.tls.cert}" = tls_locally_signed_cert.postgresql_client["gitlab"].cert_pem 
#
#        "${local.gitlab_postgresql.tls.cert2}" = tls_locally_signed_cert.postgresql_client["gitlab_user"].cert_pem 
#    }
#}
...
resource "kubernetes_secret_v1_data" "gitlab_rails_secret" {
    ...
    ## If json else yaml
    data = {
        "secrets.yml" = local.tmp_migrate_from_omnibus ? yamlencode({"production": lookup(local.gitlab_secrets, "gitlab_rails", "")}) : var.gitlab_secrets_body
    }
}
...
## If restore done in second step these need to be tainted and run again
## Add a trigger to change based on restore
