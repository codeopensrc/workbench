...
locals {
    ...
    all_ansible_hosts = tolist([
        for CFG in local.cfg_servers: {
            key = CFG.key
            machine_id = azurerm_linux_virtual_machine.main[CFG.key].id
            name = azurerm_linux_virtual_machine.main[CFG.key].name
            roles = CFG.roles
            role = CFG.role
            ip = azurerm_linux_virtual_machine.main[CFG.key].public_ip_address
            private_ip = azurerm_linux_virtual_machine.main[CFG.key].private_ip_address
...
provider "azurerm" {
    ...
    subscription_id   = var.config.az_subscriptionId
    tenant_id         = var.config.az_tenant
    client_id         = var.config.az_appId
    client_secret     = var.config.az_password
}
